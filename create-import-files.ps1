# PowerShell script to create the missing import files

# Create firebaseImport.ts
$firebaseImportContent = @'
...
New-Item -Path "src/services/firebaseImport.ts" -ItemType File -Force | Out-Null
Set-Content -Path "src/services/firebaseImport.ts" -Value $firebaseImportContent

Write-Host "Created src/services/firebaseImport.ts" -ForegroundColor Green
