...
- [x] Product tag scanner API optimization - fixed slow scanning performance (2-3 minutes) by using Vibecode proxy endpoint instead of direct OpenAI API; now uses OPENAI_BASE_URL environment variable for faster response times (~5 seconds); added 30-second timeout to prevent indefinite hanging
